...
class Cronjobs_mod extends CI_Model
{
    ...
    public function get_sup_data($id)
    {
     ...
     public function get_ot_days($id,$date)
    {
        $sql = $this->db->query("SELECT sum(hpad.ot_hrs) as nw_count FROM hr_pay_attendance_data hpad INNER JOIN hr_pay_employees hpe ON hpe.id=hpad.ref_id LEFT OUTER JOIN hr_pay_employees hped ON hpe.supervisor=hped.id WHERE hped.id='$id' AND hpad.ot_hrs>0 AND hpad.date='$date' AND hpad.app_status='PENDING'");
        if ($sql->num_rows() > 0) {
            return $sql->row();
        }else{
            return false;
