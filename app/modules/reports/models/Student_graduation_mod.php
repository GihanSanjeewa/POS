...
class Student_graduation_mod extends CI_Model
{
...
public function load_programs(){
 ...
 public function for_dropout_mockup($batch,$course){
    ...
    $this->db->where('asms_students_register.programe',$course);
    $this->db->where('asms_students_register.batch',$batch);
    $this->db->limit(5,6);
    $query=$this->db->get();
    return $query->result();
}
