...
class Ion_auth_model extends CI_Model
{
	...
	**/
    ...
    public function set_otp_login_activation($identity)
    {
        ...
        $update['otp_timestamp'] = date('Y-m-d H:i:s', time());
     ...
     **/
    public function otp_login($identity, $token, $remember=FALSE){
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, otp, otp_login_code, last_login,otp_timestamp')
        ...
        {
            $user = $query->row();
                if ($this->is_otp_token_valid($user->otp_login_code, $token,$user->otp_timestamp))
                {
                    $this->set_session($user);
        ...
        $this->trigger_events('post_login_unsuccessful');
        $this->set_error('OTP Login Unsuccessful');
     ...
     * @author Mathew and SpyTec
     **/
    public function is_otp_token_valid($stored_code, $user_token,$timestamp){
        ...
        $mins = ($now_time - $timestamp) / 60;

        if($mins < $this->otp['time_step']){
            if($stored_code==$user_token)
            {
    ...
    //TODO~~~~~~~~~Kreston~~~only~~~~~~~~~~~~~~~~~~~~~~~~~~
    public function get_companies_name($emp_id)
    {
        $this->db->select('hr_pay_m_company.name,hr_pay_m_company.id,hr_pay_m_company.code,hr_pay_m_company.address,hr_pay_m_company.logo');
        $this->db->from('aq_users');
        $this->db->join('hr_pay_m_company','hr_pay_m_company.id=aq_users.company');
        if($emp_id == 1){
            $this->db->where(array('hr_pay_m_company.id'=>1));
        }
        else{
            $this->db->where(array('aq_users.id'=>$emp_id));
        }
        $query = $this->db->get();
