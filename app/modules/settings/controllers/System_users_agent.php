<?php
class System_users_agent extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('system_users/users_agent',$data);
    ...
    public function get_all_users()
    {
        ...
        $this->datatables->where(array('asms_users_info.agent_id'=>$agent_id,'asms_users_info.id!='=>USER_ID));
    ...
    public function save()
    {
        ...
        if($this->form_validation->run()===FALSE)
        {
            ...
            $data = array(
                ...
                'agent_id'=>$this->aget_data(USER_ID)->id,
                'type'=>2,
                'user'=>USER_ID,
                'plain_pw'=>$this->input->post('password'),
            ...
            if($insert_id=$this->users->save('asms_users_info',$data)){
                ...
                if ($user_id = $this->ion_auth->register($identity, $password, $email,$additional_data,$group)) {
                   ...
                   $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('new_agent_member_user_mail', 'ion_auth'), $data, true);
                   $this->email->clear();
                   $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                   $this->email->to($email);
                   $this->email->subject("Login Details for Lead System");
