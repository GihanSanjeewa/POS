                                    ...
                                    <div class="card-body">
                                        <div class="tab-content tabcontent-border">
                                            ...
                                            <table class="df col-md-12 table" style="margin-top: 10px;width: 900px !important;">
                                                <tbody>
                                                      <input type="hidden" id="user_company" name="user_company" value="">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        user_table = $('#user_datatable').DataTable({
            ...
            "ajax": {
                ...
                "url": "<?php echo base_url()?>settings/System_users_agent/get_all_users",
                "type": "POST"
            },
    ...
    function add_user()
    {
        ...
        $.ajax({
            url: "<?php echo base_url('settings/System_users_agent/get_last_user_number'); ?>",
            type: "POST",
            dataType: "JSON",
            data: {
                "<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"
    ...
    function edit_user(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('settings/System_users_agent/edit_user')?>/" + id,
            type: "GET",
            "data": {
                "<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"
            },
    ...
    function view_user(id){
        ...
        $.ajax({
            ...
            url : "<?php echo site_url('settings/System_users_agent/view_user')?>/" + id,
            type: "POST",
            "data": {
                "<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"
            },
            ...
            success: function(data)
            {
                ...
                $.ajax({
                    url:"<?php echo site_url('settings/System_users_agent/image_available')?>",
                    dataType:"JSON",
                    type:"POST",
                    data:{
                        id:id,
    ...
    function open_photo_upload_modal(showSuccessMsg,std_id)
    {
        ...
        $('#btnUploadPhoto').on('click', function(){
            ...
            else
            {
                ...
                var url = "<?php echo site_url('settings/System_users_agent/add_photo')?>/"+idd;
...
//                    window.location = "<?php //echo base_url('hr_payroll/user_list/index')?>//";
    ...
    function save()
    {
        ...
        var url='';
        if(save_method=='add'){
            url = "<?php echo base_url('settings/System_users_agent/save'); ?>";
        }
        else{
            url = "<?php echo base_url('settings/System_users_agent/update'); ?>";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    function upload(){
        ...
        $.ajax({
            url: "<?php echo site_url('settings/System_users_agent/upload_profile')?>",
            type: "POST",
            data: {
                "<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"
            },
    ...
    function user_permissions(sys_u_p_id)
    {
        ...
        var url = "<?php echo site_url('settings/System_users_agent/ajax_get_system_permissions_details_by_id'); ?>";
            ...
            success: function(data){
                if(data.status)
                {
                    ...
                    for(var i=0; i<data.list_sections.length; i++) {
                        if(data.list_sections[i].id==11 || data.list_sections[i].id==10){
                        ...
                        for(var j=0; j<data.list_modules.length; j++) {
                            if(data.list_modules[j].id==59 || data.list_modules[j].id==87 || data.list_modules[j].id==105 || data.list_modules[j].id==86){
    ...
    function save_permissions()
    {
        ...
        var url = "<?php echo site_url('settings/System_users_agent/save_user_permissions'); ?>/";
        $.ajax({
            url: url,
            ...
            success: function(data){
                if(data.status)
                {
                    ...
                    bootbox.alert(data.message);
                    reload_table(table_System_users_agent);
                }
                else
                {
                    if(data.error)
