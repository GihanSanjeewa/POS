...
class Agent_mod extends CI_Model{
    ...
    public function get_course_types(){
      ...
      public function get_course_commision_data($id){
        $query = $this->db->query("SELECT agent_lead_commission.commission,asms_m_program_type.name as pt_name FROM agent_lead_commission left outer join asms_m_program_type on agent_lead_commission.course_type=asms_m_program_type.id where agent_lead_commission.agent_id='$id'");
        return $query->result();
    }

    //    public function get_course_commisio_data(){
    //     $this->db->select('*');
    //     $this->db->from('asms_m_program_type');
    //     $query=$this->db->get();
...
WHERE asms_lead_changes_histories.lead_id='$id'");
    ...
    public function agent_view_data_info($id){

    $query=$this->db->query("SELECT asms_lead_interest_level.name as interest_level,asms_agent_lead_changes_histories.*,TIME_FORMAT(asms_agent_lead_changes_histories.next_time, '%h:%i %p') AS convert_time,lead_follow_up_activities.name as follow_name,asms_users_info.name as username,lead_agent_detail_list.lead_temp_code as lead_id_code,lead_agent_detail_list.f_name,lead_agent_detail_list.l_name,lead_called_stutus.name as call_status_name,user.name as modified_by
FROM asms_agent_lead_changes_histories LEFT OUTER JOIN lead_follow_up_activities ON lead_follow_up_activities.id = asms_agent_lead_changes_histories.follow_up

LEFT OUTER JOIN lead_agent_detail_list ON lead_agent_detail_list.id = asms_agent_lead_changes_histories.lead_id
LEFT OUTER JOIN asms_users_info ON lead_agent_detail_list.system_id=asms_users_info.id
LEFT OUTER JOIN asms_users_info as user ON asms_agent_lead_changes_histories.user=user.id
LEFT OUTER JOIN asms_lead_interest_level ON asms_lead_interest_level.id=asms_agent_lead_changes_histories.inter_level
LEFT OUTER JOIN  lead_called_stutus ON  lead_called_stutus.id = asms_agent_lead_changes_histories.call_status
WHERE asms_agent_lead_changes_histories.lead_id='$id'");
        // $query=$this->db->get();
...
WHERE asms_loan_changes_histories_data.lead_id='$id'");
    ...
    public function get_users(){
       ...
       public function get_relevant_lead($id){
        ...
        $this->db->where('lead_agent_detail_list.id',$id);
        $query=$this->db->get();
...
if ($this->ion_auth->in_group($groups_1)) {
    ...
    {
        $query=$this->db->query("SELECT lead_agent_detail_list.lead_temp_code,lead_agent_detail_list.id,lead_agent_detail_list.int_level
        FROM lead_agent_detail_list
        WHERE lead_agent_detail_list.id='$lead_id'");
           return $query->result();
    }
...
/////past student
 ...
 function get_agent_details_all()
    {

        // $sql = "SELECT * FROM payments_main WHERE payments_main.app_status ='FC 1 Pending' OR payments_main.app_status='FC 4 Approved'";
        $sql = "SELECT *,lead_agent_detail_list.id as agnId,asms_lead_interest_level.name as int_name FROM lead_agent_detail_list LEFT JOIN asms_m_countries on asms_m_countries.id=lead_agent_detail_list.country left outer join asms_lead_interest_level on lead_agent_detail_list.int_level=asms_lead_interest_level.id  where lead_agent_detail_list.lead_status!='Approved' and lead_agent_detail_list.lead_type='External'";
        ...
        if ($res->num_rows() > 0) {
            foreach (($res->result()) as $row) {
                $data[] = array(
                    "id" => $row->agnId,
                    "lead_temp_code" => $row->lead_temp_code,
                    "copany_name" => $row->copany_name,
                    "f_name" => $row->f_name,
                    "phone" => $row->phone,
                    "email" => $row->email,
                    "city" => $row->city,
                    ...
                    "last_contact" => $row->last_call_date.' '.$row->last_call_time,
                    "next_contact" => $row->next_contact_date.' '.$row->next_contact_time,
     ...
     function get_agent_details_all_internal()
    {

        // $sql = "SELECT * FROM payments_main WHERE payments_main.app_status ='FC 1 Pending' OR payments_main.app_status='FC 4 Approved'";
        $sql = "SELECT *,lead_agent_detail_list.id as agnId,asms_lead_interest_level.name as int_name FROM lead_agent_detail_list LEFT JOIN asms_m_countries on asms_m_countries.id=lead_agent_detail_list.country left outer join asms_lead_interest_level on lead_agent_detail_list.int_level=asms_lead_interest_level.id where lead_agent_detail_list.lead_status!='Approved' and lead_agent_detail_list.lead_type='Internal'";
    ...
    function get_approved_agent_details()
    {

        // $sql = "SELECT * FROM payments_main WHERE payments_main.app_status ='FC 1 Pending' OR payments_main.app_status='FC 4 Approved'";
        $sql = "SELECT *,lead_agent_detail_list.id as agnId FROM lead_agent_detail_list LEFT JOIN asms_m_countries on asms_m_countries.id=lead_agent_detail_list.country where lead_agent_detail_list.lead_status='Approved'";
    ...
    public function get_temp_agent_details_view($id)
    {
        $this->db->select('*');
        $this->db->from('lead_agent_detail_list');
     ...
     public function get_approved_agent_details_view($id)
    {
      ...
      public function get_agent_attachments($id)
    {
        $this->db->select('*');
        $this->db->from('asms_lead_agent_data_att');       
        $this->db->where("asms_lead_agent_data_att.main_id",$id);
        $query=$this->db->get();
        return $query->row();
    }


     public function get_agent_detailss_one($id)
    {
        $this->db->select('*');
        $this->db->from('lead_agent_detail_list');
        $this->db->join('asms_m_countries','asms_m_countries.id=lead_agent_detail_list.country','left');
    ...
    public function get_maxId()
    {
        $this->db->select('*');
        $this->db->from('lead_agent_detail_list');
        $this->db->order_by("lead_agent_detail_list.id", "DESC");
        $query = $this->db->get();
    ...
    public function get_agent_maxId()
    {
        $this->db->select('*');
        $this->db->from('lead_agent_detail_list');
        $this->db->order_by("lead_agent_detail_list.code_number", "DESC");
        $query = $this->db->get();
      ...
      public function get_duplicate($email_data)
    {
        $this->db->select('*');
        $this->db->from('lead_agent_detail_list');       
        $this->db->where("lead_agent_detail_list.email",$email_data);
        $query=$this->db->get();
        return $query->row();
    }
